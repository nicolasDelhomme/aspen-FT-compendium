...
hier.clust <- function (data.loc, samples.sep.loc, rnames = FALSE) {
  ...
  annot_col <- data.frame(S2        = samples.sep.loc$S2,
...
data.mod <- data.mod[, reorder_idx]
  ...
  samples.sep_hm$S2 <- factor(ifelse(samples.sep_hm$S2 %in% c("Y", "A1", "A2"),
                            samples.sep_hm$S2, NA), levels = c("Y", "A1", "A2"))

  annot_col <- data.frame(Tissues   = samples.sep_hm$Tissue,
  ...
  annot_colors = list(Location = location_map,
    Tissues   = c(Bud = "chocolate", Leaf = "darkgreen"),
     `Month/Treatment` = c(
        SEP = "#F3766E",
        OCT = "#D98F28",
        DEC = "#2CBAA4",
...
#pdf(file = "Figures/Transcriptomics_Torgeir/heatmap_modules.pdf")
